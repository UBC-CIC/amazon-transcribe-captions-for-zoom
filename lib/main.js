// Most of this code originates from https://github.com/aws-samples/amazon-transcribe-websocket-static
...
const http_cors_proxy_api = appconfig.http_cors_proxy_api;
...
$('#start-button').click(function () {
    // hide any existing errors and successes
    $('#error').hide();
    $('#success').hide();
...
let streamAudioToWebSocket = function (userMediaStream) {
    ...
    setupNewSocket(micStream);
}

let setupNewSocket = function(micStream) {
    preSingedUrl(languageCode).then( function(url) {
...
function wireSocketEvents(micStream) {
    let socketError = false;
    let transcribeException = false;
    let restartSocketOnError = false;

    // when we get audio data from the mic, send it to the WebSocket if possible
    socket.onopen = function() {
        showSuccess('<i class="fa fa-microphone"></i> <strong>Captioning in progress.</strong> Keep this page open until you are done.');
    ...
    socket.onmessage = function (message) {
        ...
        if (messageWrapper.headers[":message-type"].value === "event") {
            ...
            if (messageBody.Message == 'Your request timed out because no new audio was received for 15 seconds.') {
                restartSocketOnError = true;
            } else {
...
let handleEventStreamMessage = function (messageJson) {
    ...
    if (results.length > 0) {
        if (results[0].Alternatives.length > 0) {
            ...
            if ($('#transcript')[0].value !== undefined) {
            ...
            if (!results[0].IsPartial) {
                ...
                if (zoom_endpoint !== ''){
                    ...
                    showError('There was an error sending your transcript to Zoom. No Zoom API Token set.');
                    return
                }

                let zoom_end_point_url = http_cors_proxy_api + '?' + zoom_token + '&seq='+ seq_num + '&lang=' + languageCode;
                axios({
                    method: 'POST',
                ...
                .catch(function (error) {
                    ...
                    showError('There was an error sending your transcript to Zoom. Please try copying the token from Zoom again.');
                });
                seq_num++;
            }
        }
